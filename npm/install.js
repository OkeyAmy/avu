...
async function verifyArchive(archive, asset, checksumsPath) {
  ...
  const line = checksums.find(entry => entry.trim().split(/\s+/)[1] === asset);
...
async function main() {
  ...
  const archive = join(nativeDir, basename(selected.asset));
  ...
  rmSync(checksums, { force: true });
}

main().catch(error => {
  console.error(error.message);
  console.error("Avu normal installs use Node/npm plus a prebuilt release binary.");
  console.error("Rust/Cargo is only for contributors building from source.");
  process.exit(1);
});
